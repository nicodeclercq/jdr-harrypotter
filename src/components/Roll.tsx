...
export function Roll<T extends Dice.Dice[]>({
  dices,
  concat,
  onRollEnd,
}: Props<T>) {
  ...
  const onResult = useCallback(
    () =>
      onRollEnd(concat(values.map(({ values }) => values[values.length - 1]))),
    [concat, onRollEnd, values]
  );
  ...
  return (
    <DoRoll
      dices={values.map(({ type, values }) => ({ type, value: values[index] }))}
    />
  );
}
