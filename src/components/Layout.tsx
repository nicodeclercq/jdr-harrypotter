...
function NavLink({
  ...
  const style = {
    active: `bg-gradient-to-r ${getColor(
      "secondary",
      600,
      "from:background"
    )} ${getColor("secondary", 400, "to:background")} ${getColor(
      "secondary",
      700,
      "hover:background"
    )} border-r-2 border-white`,
    inactive: `${getColor("secondary", 800)}  ${getColor(
      "secondary",
      700,
      "hover:background"
    )}`,
  ...
  return (
    <Link to={path}>
      <div
        className={`flex text-4xl flex-row space-x-2 py-2 px-4 items-center ${
          match ? style.active : style.inactive
        }`}
      >
        {typeof icon === "string" ? (
...
export function Layout({ children }: { children: React.ReactNode }) {
      ...
      fromRemoteData(({ lockKeys, state, role }) => (
        <div
          className={` ${getColor(
            "secondary",
            800
          )} fixed h-full text-white divide-y divide-blue-500 flex flex-col`}
          style={{ zIndex: 2 }}
        >
  ...
  return (
    <>
      <div
        className="flex h-screen overflow-y-auto bg-gray-500"
