...
const isStringValue = (a: FormValue): a is StringValue =>
  typeof a.defaultValue === "string" && !("values" in a);
const isNumberValue = (a: FormValue): a is NumberValue =>
  typeof a.defaultValue === "number" && !("values" in a);
...
export function Form<T extends Fields<Key>, Key extends string>({
  template,
  onCancel,
  fields,
  onSubmit,
  ...
  const submitHandler = useMemo(
    () =>
      handleSubmit((record: FieldValues) =>
        onSubmit(record as ValuesFromDefintion<T>)
      ),
  ...
  const StringInput = useCallback(
    <K extends Path<T>>({
      ...
      <Controller
        ...
        rules={{
          required: config.isRequired,
          validate: config.validate as Validate<PathValue<T, K>, T>,
        }}
        render={({ field: { value, onChange } }) => (
          <>
            <Input
    ...
    return template
      ...
      .map((row) => {
        ...
        return row
          .map(
            (fieldName) =>
              `label-${fieldName as string} ${createArray(
                colSpan,
