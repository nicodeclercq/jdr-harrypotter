...
type Props<T extends string> = BaseProps<T> & (MultipleValuesProps | SingleValueProps);

const isMulti = <T extends string>(props: Props<T>): props is BaseProps<T> & MultipleValuesProps => props.multiple === true;

export function Select<T extends string>(props: Props<T>){
  const {id, onChange, options, value, width, theme = 'base', multiple = false} = props;
  ...
  return (
    <select
      ...
      onChange={(e) => {
        ...
        const values = Array.from(e.target.querySelectorAll('option:checked'))
