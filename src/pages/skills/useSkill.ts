...
export const useSkill = () => {
  ...
  const add = (name: string, currentLevel: number) =>
    pipe(
      skills,
  ...
  const edit = (name: string, currentLevel: number) =>
    pipe(
      skills,
      RemoteData.chain((skills) =>
        name in skills
  ...
  const remove = (name: string) =>
    pipe(
      skills,
      RemoteData.map((skills) => Objects.remove(name.trim(), skills)),
      onSuccess(setSkills)
    );
    ...
    pipe(
      ...
      RemoteData.map((skills) => {
        ...
        const nextLevel = pipe(
          ...
          Interaction.fold({
            success: (points) =>
              skills[skill].currentLevel + points < MAX_SKILL_LEVEL
                ? currentLevel + points
                : MAX_SKILL_LEVEL,
