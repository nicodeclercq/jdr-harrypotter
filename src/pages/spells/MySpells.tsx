...
import {
  getNextLevelSpells,
  UserSpells as UserSpellsType,
} from "./domain/UserSpell";
...
function UserSpells({
  userSpells,
  goTo,
}: { userSpells: UserSpellsType } & Props) {
  const [rollModalSpell, setRollModalSpell] = useState<
    SpellType.Spell | undefined
  >(undefined);
  const [nextLevelSpell, setNextLevelSpell] = useState<
    SpellType.Spell | undefined
  >(undefined);
  ...
  const onRollEnd =
    ...
    (result: Interaction.Interaction<never, number>) => {
      pipe(
        result,
        Interaction.fold({
          ...
          if (nextLevelSpells.length) {
            nextLevelSpells.forEach((nextLevelSpell) =>
              add({
                ...
                type: "success",
                message: `“${
                  nextLevelSpell.spell?.incantation ?? nextLevelSpell.name
                }” peut être amélioré`,
                action: {
                  run: () => {
  ...
  return (
    <>
        ...
        {knownSpells.length ? (
          knownSpells.map(({ spell }) => (
            <Spell
              ...
              actions={
                <Button
                  onClick={() => remove(spell as SpellType.Spell)}
                  type="secondary"
                >
                  Supprimer -
                </Button>
              ...
              isOwned
              currentLevel={
                userSpells.knownSpells[spell?.name ?? ""]?.currentLevel
              }
            />
          ))
        ) : (
          <EmptyContent goTo={goTo}>
            {{
              ...
              description:
                "Il va falloir te mettre au travail vite fait mon petit gars!",
            }}
          </EmptyContent>
        )}
      </Card>
      {rollModalSpell != null && (
        <RollModal
          successPercentage={
            userSpells.knownSpells[rollModalSpell.name].currentLevel
          }
      ...
      {nextLevelSpell != null && (
        <UpgradeRollModal
          successPercentage={
            userSpells.knownSpells[nextLevelSpell.name].currentLevel
          }
...
export function MySpells({ goTo }: Props) {
  ...
  return pipe(
    ...
    fromRemoteData((userSpells) => (
      <UserSpells goTo={goTo} userSpells={userSpells} />
    ))
  );
}
