...
export function ObjectsForm({objects, columns, maxDisplayed}: {objects: Record<string, number>, columns: 1 | 2, maxDisplayed?: number | undefined}){
  ...
  const { handleSubmit, control, errors } = useForm<{objects: {name: string, number: number}[]}>({
    defaultValues: {
    ...
    resolver: (values: {objects: {name: string, number: number}[]}) => {
      ...
      if(hasDupplicates) {
        ...
        return {
          values: {},
          errors: fromEntries(dupplicates.map(name => [
  ...
  const onSubmit = ({objects }: {objects: {name: string, number: number}[]}) => {
    pipe(
      objects
      ...
      <Card title="Objets">
        <div className={`grid gap-2 ${fields.length === 0  || columns === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
          ...
          }
          {
            fields.map((field, index) => (
              <div key={field.id} className="flex flex-row items-start p-2 space-x-2">
                <Controller
                ...
                <Controller
                  ...
                  control={control}
                  rules={{ required: true }}
                  render={({value, onChange, name}) => (<div className="flex flex-col">
        ...
        </div>
        {
          ...
          <div className="flex justify-center p-2 flex-cols space-x-2">
            <Button type="primary" onClick={() => append({name: '', number: 1})}>Ajouter</Button>
          </div>
        }
        {
          maxDisplayed != null && fields.length > 0 &&
          <div className="flex justify-center p-2 flex-cols space-x-2">
            <Button type="secondary" onClick={() => goTo('/objects')}>Voir tous les objets</Button>
          </div>
        }
      </Card>
    </form>
