...
function VerbNumber({
  ...
  onSubmit: (result: { value: string; invert: string }) => void;
}) {
...
export function NumberDefinitionForm({
  onSubmit,
}: {
  onSubmit: (result: NumberType) => void;
}) {
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      {type === "name" ? (
        <NameNumber
          onSubmit={(result) => setValue({ type: "name", ...result })}
        />
      ) : type === "verb" ? (
        <VerbNumber
          onSubmit={(result) => setValue({ type: "verb", ...result })}
        />
      ) : type === "inverse" ? (
        <InverseNumber onSubmit={() => setValue({ type: "inverse" })} />
      ) : (
        <></>
      )}
      <Button type="primary" onClick={() => onSubmit(value as NumberType)}>
        Valider
      </Button>
    </div>
  );
