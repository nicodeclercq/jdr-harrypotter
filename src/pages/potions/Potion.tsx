...
export function Potion({ number, emptyBottles, ownedIngredients, potion, isOwned = false }: Props) {
    ...
    <>
      <Accordion>
        {{
          ...
          content: <div className="flex flex-col mb-4 space-y-4">
            <div className="flex-grow text-left capitalize space-x-2 space-y-2">
              <hr className="border-y"/>
              <div><BodyText>{potion.description}</BodyText></div>
              {!isOwned &&
                    ...
                    values={[potion.ingredients.map(ingredient => (<div className="space-x-2">
                      {
                        hasIngredient(ingredient) 
                          ? <span className="text-green-600"><Icon name="CHECK" /></span>
                          : <span className="text-red-700"><Icon name="CROSS" /></span>
                      }
