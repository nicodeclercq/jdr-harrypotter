...
const potionsLens = stateLens.fromProperty("potions");

export const usePotions = () => {
  const [potions, setPotions] = useStore(potionsLens);

  const getCookedPotions = () =>
    pipe(
      potions,
      RemoteData.map((potions) => potions.cookedPotions)
    );

  const getOwnedIngredients = () =>
    pipe(
      potions,
      RemoteData.map((potions) => potions.ingredients)
    );

  const setOwnedIngredientsAndBottles = (
    ingredients: State["potions"]["ingredients"],
    emptyBottles: State["potions"]["emptyBottles"]
  ) =>
    pipe(
      potions,
      onSuccess((potions) =>
  ...
  const cookPotion = (potionName: string) => {
    return pipe(
      ...
      RemoteData.map((potions) => {
        ...
        const cookedPotions = potions.cookedPotions.find(
          (cookedPotion) => cookedPotion.name === potionName
        )
          ? potions.cookedPotions.map((cookedPotion) =>
              cookedPotion.name === potionName
        ...
        const ingredients = potions.ingredients.map((ingredient) => {
          const containsIngredient =
            potion?.ingredients.find((i) => i === ingredient.name) != null;
        ...
        return {
          ...
          cookedPotions,
          ingredients,
          emptyBottles: potions.emptyBottles > 0 ? potions.emptyBottles - 1 : 0,
        };
      }),
