...
export const commonMagical = magicalObjects.filter(
  ({ cost, knowledge }) => cost < 100 || knowledge > 60 || knowledge <= 30
);
export const usualMagical = magicalObjects.filter(
  ({ cost, knowledge }) =>
    (cost >= 100 && cost < 1000) || (knowledge > 30 && knowledge <= 60)
);
export const rareMagical = magicalObjects.filter(
  ({ cost, knowledge }) => cost >= 1000 || (knowledge > 30 && knowledge <= 60)
);

export const common = normalObjects.filter(({ cost }) => cost < 50);
export const usual = normalObjects.filter(
  ({ cost }) => cost >= 50 && cost < 80
);
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="p-1 bg-gray-200 rounded space-x-2">
              {initialCost !== cost && (
                <div className="flex">
                  <Money value={initialCost} />
                </div>
              )}
              <div className="flex p-1 bg-gray-300 rounded transform scale-75">
                <Money value={Math.round(initialCost / 2)} />
              </div>
            </div>
          </div>
        </div>
...
const getPoorIngredients = () =>
  getNRandomFromArray(
    5,
    ingredients.filter(({ scarcity }) => scarcity === "Commun")
  );
const getNormalIngredients = () =>
  getNRandomFromArray(
    4,
    ingredients.filter(
      ({ scarcity }) => scarcity === "Commun" || scarcity === "Usuel"
    )
  );
const getRichIngredients = () =>
  getNRandomFromArray(
    3,
    ingredients.filter(
      ({ scarcity }) => scarcity === "Insolite" || scarcity === "Très rare"
    )
  );
      ...
      <div className="flex w-full h-full gap-2">
        <div className="w-1/3">
          <Card title="Objets magiques" useDividers>
            {magicalObjects
              ...
              .map(({ id, cost, name, description, knowledge, side }) => (
                <Item
                  key={name}
                  id={id}
                  initialCost={cost}
        ...
        <div className="flex flex-col flex-grow gap-2">
            ...
            title={
              <div className="flex justify-between">
                ...
                <Button
                  type="secondary"
                  onClick={() => {
                    setPoor(getPoorItems());
                    setPoorIngredients(getPoorIngredients());
                  }}
                >
                  <Icon name="DICE" />
                </Button>
          ...
          >
            ...
            {poorIngredients.map(({ name, scarcity }) => (
              <IngredientItem
                key={name}
                percent={5}
                name={name}
          ...
          <Card
            title={
              <div className="flex justify-between">
                ...
                <Button
                  type="secondary"
                  onClick={() => {
                    setNormal(getNormalItems());
                    setNormalIngredients(getNormalIngredients());
                  }}
                >
                  <Icon name="DICE" />
                </Button>
            ...
            useDividers
          >
            {normal.map(({ id, cost, name, description, knowledge, side }) => (
              <Item
                key={name}
                id={id}
                initialCost={cost}
                cost={addRandomPercent(10, cost)}
                description={description}
                knowledge={knowledge}
                name={name}
                side={side}
            ...
            {normalIngredients.map(({ name, scarcity }) => (
              <IngredientItem
                key={name}
                percent={10}
                name={name}
            ...
            title={
              <div className="flex justify-between">
                ...
                <Button
                  type="secondary"
                  onClick={() => {
                    setRich(getRichItems());
                    setRichIngredients(getRichIngredients());
                  }}
                >
                  <Icon name="DICE" />
                </Button>
          ...
          >
            ...
            {richIngredients.map(({ name, scarcity }) => (
              <IngredientItem
                key={name}
                percent={20}
                name={name}
