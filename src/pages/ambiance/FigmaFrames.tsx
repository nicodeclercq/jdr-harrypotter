...
type Images = Record<string, RemoteData.RemoteData<Error, {image: string, name: string}[]>>;

export function FigmaFrames({file}: Props) {
  const [images, setImages] = usePersistantState<Images>('AMBIANCE_IMAGES', {});
  const [selectedView, setSelectedView] = usePersistantState<string | undefined>('AMBIANCE_SELECTED_IMAGE', undefined);
  ...
  const fetch = useCallback((file: string) => {
    ...
    getFrames(file.trim())
      ...
      .catch((e) => {
        console.error(e);
        setImages({...images, [file]: RemoteData.failure(new Error('Can\'t get frames from given file'))});
      });
            ...
            fromRemoteData((images) => <>{
              images.map(({image, name}) => (
                <div key={name}>
                  ...
                  <div className="flex justify-center m-1 mt-0 text-white bg-gray-700 border border-t-0 border-white rounded-b" style={{width:'25vmin'}}><span style={{textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>{name}</span></div>
                </div>
              ))
              }</>,
              () => <></>
