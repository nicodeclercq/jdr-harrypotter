...
export function AmbiancePage() {
  ...
  const [file, setFile] = usePersistantState<string | undefined>(
    "AMBIANCE_LINK",
    undefined
  );
    ...
    <Layout>
      ...
      >
        <Card fullWidth title="Images" grow>
          <div
            className="flex flex-col p-2 overflow-scroll space-x-2"
            style={{ height: "66vh" }}
          >
            <Form
              onSubmit={onSubmit}
              fields={{
                file: { defaultValue: file ?? "", label: "Lien figma" },
              }}
              submitOnBlur
            />
        ...
        <Card
          ...
          title={
            <div className="flex items-center space-x-2">
              ...
              <Button
                type="secondary"
                onClick={() => setShowMusicModal({ name: "", url: "" })}
              >
                <Icon name="INCREASE" />
              </Button>
            </div>
                ...
                (data) => (
                  <>
                    {data.map(({ name, url }) => (
                      <div
                        key={name}
                        className="flex flex-row items-center w-full p-2 space-x-2"
                      >
                        <ButtonIcon
                          icon="PEN"
                          onClick={() => setShowMusicModal({ name, url })}
                        ...
                        <Button
                          type="secondary"
                          onClick={onClickOnMusic(name, url)}
                        >
