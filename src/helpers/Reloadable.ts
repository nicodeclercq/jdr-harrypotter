...
export const isReloading = <E, T>(data: Reloadable<E, T>): data is Reloading<T> => '_tag' in data && data._tag === TAG;

export const map = <E, A, B>(fn: FunctionN<[A], B>) => (reloadable: Reloadable<E, A>): Reloadable<E, B> => {
...
export const chain = <E, A, B>(fn: FunctionN<[A], Reloadable<E, B>>) => (reloadable: Reloadable<E, A>): Reloadable<E, B> => {
  if(isReloading(reloadable)){
    return pipe(
      reloadable.value,
      fn,
