...
export type TypeofItems<U extends unknown[]> = U extends (infer R)[]
  ? R
  : never;
...
export const findLastIndex = <T>(
  ...
  arr: T[]
): number => arr.length - 1 - arr.reverse().findIndex(predicate);

export const getNRandomIndexFromFilteredArray = <T>(
  number: number,
  ...
  return [
    indexes[randomIndex],
    ...getNRandomIndexFromFilteredArray(
      number - 1,
      (a: T, index: number) => index !== randomIndex && predicate(a, index),
      arr
    ),
  ];
};
...
export const getNRandomFromArray = <T>(number: number, arr: T[]): Array<T> => {
  ...
  if (number >= arr.length) {
    return new Array(number).fill(0).map((_, i) => arr[i]);
  }
...
export const createEmptyArray = <T>() => [] as T[];
